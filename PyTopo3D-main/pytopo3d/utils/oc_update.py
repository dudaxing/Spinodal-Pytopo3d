...
def optimality_criteria_update(
    ...
    # NOTE: parenthesized so that when CuPy is absent we never touch cp/cusp
    # (the original precedence raised NameError on CPU-only installs).
    inputs_on_gpu = HAS_CUPY and (
        any(isinstance(arr, cp.ndarray) for arr in [x, dc, dv, obstacle_mask, Hs])
